...
def train(global_step=0):
    ...
    for epoch in range(first_epoch, args.num_train_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            ref_image = torch.concat([ref_image, ref_image], dim=0)
            ...
            prompt_embeds = torch.concat([prompt_embeds, prompt_embeds], dim=0)
            ...
            with accelerator.accumulate(transformer):
                ...
                with torch.no_grad():
                    ref_pred = ref_model(latent_model_input, prompt_embeds, timesteps,
                                             img_ids, text_ids, guidance, return_dict=False)[0]
                    ref_pred = ref_pred[:, :packed_noisy_latents.size(1)]
